...
class AopCertification
{
     ...
     */
    static public function isTrusted($alipayCert, $rootCert)
    {
    ...
    static public function verifySignature($alipayCert, $rootCert)
    {
    ...
    static public function readPemCertChain($cert)
    {
    ...
    static public function verifyCert($prev, $rootCerts)
    {
        $nowTime = time();
        ...
        for ($i = 0; $i < count($rootCerts); $i++) {
            $subjectDN = AopCertification::array2string($rootCerts[$i]['subject']);
            $subjectMap[$subjectDN] = $rootCerts[$i];
        }
        $issuerDN = AopCertification::array2string(($prev['issuer']));
     ...
     */
    static public function verifyCertChain($alipayCerts, $rootCerts)
    {
     ...
     * @param $certs 证书链
     */
    static public function sortByDn(&$certs)
    {
        //是否包含自签名证书
        ...
        for ($i = 0; $i < count($certs); $i++) {
            ...
            $subjectDN = AopCertification::array2string($certs[$i]['subject']);
            $issuerDN = AopCertification::array2string(($certs[$i]['issuer']));
     ...
     * @param $cert 目标证书
     */
    static public function isSelfSigned($cert)
    {
        $subjectDN = AopCertification::array2string($cert['subject']);
        $issuerDN = AopCertification::array2string($cert['issuer']);
     ...
     */
    static public function addressingUp($subjectMap, &$certChain, $current)
    {
        $certChain[] = $current;
        if (isSelfSigned($current)) {
            return;
        }
        $issuerDN = AopCertification::array2string($current['issuer']);
     ...
     */
    static public function addressingDown($issuerMap, &$certChain, $current)
    {
        $subjectDN = AopCertification::array2string($current['subject']);
     ...
     */
    static public function extractSignature($der = false)
    {
        if (strlen($der) < 5) {
            return false;
        }
     ...
     * @return string oid
     */
    static public function getSignatureAlgorithmOid($der = null)
    {
     ...
     * @return string hash
     */
    static public function getSignatureHash($der = null)
    {
        // Validate this is the der we need...
        if (!is_string($der) or strlen($der) < 5) {
            return false;
     ...
     * @return bool
     */
    static public function isCertSigner($certPem = null, $caCertPem = null)
    {
        if (!function_exists('openssl_pkey_get_public')) {
            die('Need the openssl_pkey_get_public() static public function.');
        }
        if (!function_exists('openssl_public_decrypt')) {
            die('Need the openssl_public_decrypt() static public function.');
        }
        if (!function_exists('hash')) {
            die('Need the php hash() static public function.');
        }
     ...
     * @return bool false on failures
     */
    static public function pemToDer($pem = null)
    {
        if (!is_string($pem)) {
            return false;
        }
     ...
     * @return bool false on failures.
     */
    static public function stripSignerAsn($der = null)
    {
